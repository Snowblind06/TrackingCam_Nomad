...
while video.isOpened():
    ...
    for cnt in contours:
        ...
        if cv2.contourArea(cnt) > 700 and (x <= 840) and 150 <= y <= 350:
            cv2.rectangle(Prev_frame, (x, y), (x + w, y + h), (255, 255, 0), 3)
            motion += 1
            cv2.putText(Prev_frame, f'person {motion} area {cv2.contourArea(cnt)}', (x, y), cv2.FONT_HERSHEY_SIMPLEX,
                        0.5, (0, 0, 255), 1)
    # ROI definition
    cv2.line(Prev_frame, (0, 220), (840, 220), (0, 0, 255), 4)
    cv2.line(Prev_frame, (0, 450), (840, 450), (0, 0, 255), 4)
